    ...
    if (!utils.noEmptyParams(req.body)) res.json({success: false, message: config.messages.NO_DATA});
        ...
        User.isEmailUnique(email)
            ...
            .then(() => {
                ...
                utils.info(`User '${firstName} ${lastName}' registered!`);
            })
            .catch(error => {
                utils.error(error);
...
module.exports.info = (req, res) => {
    ...
    User.findById(user._id) //_id should remain the same
        .then(user => {
            utils.info(`User '${user.firstName} ${user.lastName}' got info!`);
...
module.exports.update = (req, res) => {
    ...
    User.findByIdAndUpdate(user._id, {$set: set})
        .then(() => {
            utils.info(`User '${user.firstName} ${user.lastName}' updated!`);
            res.json({success: true});
        })
        .catch(error => {
            utils.error(error);
...
module.exports.delete = (req, res) => {
    ...
    User.findByIdAndRemove(user._id)
        .then(() => {
            utils.info(`User '${user.firstName} ${user.lastName}' deleted!`);
            res.json({success: true});
        })
        .catch(error => {
            utils.error(error);
